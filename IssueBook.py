...
allteachId = [] #List To store all teacher IDs
...
def issue():
    ...
    extractteachID = "select rollno from "+teachTable
    try:
        cur.execute(extractteachID)
        con.commit()
        for i in cur:
            allteachId.append(i[0])
        
        if issueby in allteachId:
            pass
        else:
            messagebox.showinfo("Error","teach ID not present")
    except:
        messagebox.showinfo("Error","Can't fetch teach IDs")
    ...
    try:
        if bid in allBid and issueto in allRoll and issueby in allteachId and status == True:
...
def issueBook(): 
    ...
    # Issued By teacher Number
    lb3 = Label(labelFrame,text="Issued By(teachid) : ", bg='black', fg='white')
